 ...
 * container. Makes {@link HttpServletRequest}, {@link HttpServletResponse} and {@link
 * ServletContext} available to web-app specific SiteMesh components.
 *
 * @author Joe Walnes
...
public class WebAppContext extends BaseSiteMeshContext {
  ...
  /**
   * Key that the {@link WebAppContext} is stored under in the {@link HttpServletRequest} attribute.
   * It is "org.sitemesh.SiteMeshContext".
   */
  public static final String CONTEXT_KEY = SiteMeshContext.class.getName();

  /**
   * The name of the request attribute under which the original servlet path is made available to
   * the target of a forward
   */
  ...
  public static String getRequestPath(HttpServletRequest request) {
    ...
    if (request.getAttribute(FORWARD_SERVLET_PATH) != null) {
      ...
      String requestPath = requestUrl.substring(requestUrl.indexOf('/',requestUrl.indexOf('/',requestUrl.indexOf('/')+1)+1));
      if (request.getContextPath().length() == 0) {
        result = requestPath;
      } else {
        result = requestPath.substring(request.getContextPath().length());
      }
    }
   ...
   * looking them up as {@link HttpServletRequest} attributes under the keys {@link #CONTENT_KEY}
   * and {@link #CONTEXT_KEY} respectively.</p>
   */
  @Override
  protected void decorate(String decoratorPath, Content content, Writer out) throws IOException {
    ...
    HttpServletResponseBuffer responseBuffer = new HttpServletResponseBuffer(response, metaData,
        new BasicSelector(new PathMapper<Boolean>(), includeErrorPages) {
          @Override
          public boolean shouldBufferForContentType(String contentType, String mimeType,
              String encoding) {
  ...
  /**
   * Dispatch to the actual path. This method can be overriden to provide different ways of
   * dispatching (such as cross web-app).
   */
